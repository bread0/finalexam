...
void draw() {
  ...
  if (mousePressed && dist(mouseX, mouseY, width/4, height/4) < 50) {
    on1 = true;
  }
  if (mousePressed && dist(mouseX, mouseY, width/4, 3 * height/4) < 50) {
    on2 = true;
  }
  if (mousePressed && dist(mouseX, mouseY, 3 * width/4, height/4) < 50) {
    on3 = true;
  }
  if (mousePressed && dist(mouseX, mouseY, 3 * width/4, 3 * height/4) < 50) {
    on4 = true;
  }
  if (mousePressed && dist(mouseX, mouseY, width/4, height/4) > 50) {
    on1 = false;
